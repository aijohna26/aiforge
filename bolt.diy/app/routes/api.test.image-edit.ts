...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (!prompt || typeof prompt !== 'string') {
      return json({ error: "Please provide a 'prompt' describing the edits" }, { status: 400, headers });
    }

    if (!imageUrl || typeof imageUrl !== 'string') {
      return json({ error: "Please provide an 'imageUrl' to edit" }, { status: 400, headers });
    }
    ...
    try {
      ...
      console.log(
        `[Image Edit] Image edited successfully${isDevelopmentMode ? ' (dev mode - no credits charged)' : `, ${cost} credits deducted`}`,
      );
