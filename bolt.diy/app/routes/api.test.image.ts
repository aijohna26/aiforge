...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (!prompt || typeof prompt !== 'string') {
      return json({ error: "Please provide a 'prompt'" }, { status: 400, headers });
    }
    ...
    const migrateImage = async (url: string) => {
      ...
      try {
        ...
        const { data, error: uploadError } = await supabase.storage.from('images').upload(fileName, buffer, {
    ...
    try {
      ...
      console.log(
        `[Image Test] Image generated successfully${isDevelopmentMode ? ' (dev mode - no credits charged)' : `, ${cost} credits deducted`}`,
      );
