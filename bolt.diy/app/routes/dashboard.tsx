        ...
        <div className="min-h-screen bg-[#FAFAFA] dark:bg-[#080808] text-gray-900 dark:text-white transition-colors duration-500 overflow-x-hidden relative">
            ...
            <nav className="relative z-10 flex items-center justify-between px-8 py-6 max-w-7xl mx-auto backdrop-blur-sm">
                ...
                <div className="flex items-center gap-6">
                    <Link to="/chat" className="text-sm font-bold text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition-colors">
                        Editor
                    </Link>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-accent-500/10 flex items-center justify-center border border-accent-500/20">
                                <span className="text-xs font-black text-accent-600 dark:text-purple-400">{user?.email?.[0].toUpperCase()}</span>
                            </div>
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
                    ...
                    >
                        <h1 className="text-5xl md:text-6xl font-black tracking-tight mb-4">
                            Your <span className="bg-clip-text text-transparent bg-gradient-to-r from-accent-500 to-orange-500">Forge</span>
                        </h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {projects.length === 0 ? (
                        ...
                        >
                            ...
                            <h3 className="text-2xl font-black text-gray-400 dark:text-gray-500 mb-2 uppercase tracking-tight">Empty Forge</h3>
                            <p className="text-gray-400 dark:text-gray-600 mb-8 max-w-xs font-bold uppercase text-[10px] tracking-[0.2em]">No projects discovered in your workshop yet.</p>
                            <Button onClick={handleCreateNew} className="bg-accent-500 text-white font-black px-8 py-4 rounded-2xl scale-110">
                                Craft First App
                            </Button>
                        </motion.div>
                    ) : (
...
function ProjectCard({ project, index, navigate, onDelete }: { project: Project, index: number, navigate: any, onDelete: () => void }) {
    const logoUrl = project.data?.step3?.logo?.url;
    const displayLogoUrl = logoUrl ? (logoUrl.startsWith('http') ? `/api/image-proxy?url=${encodeURIComponent(logoUrl)}` : logoUrl) : null;
