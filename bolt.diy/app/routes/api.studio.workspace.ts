...
export async function action({ request }: ActionFunctionArgs) {
    ...
    let query = supabase.from('studio_workspaces').upsert(workspaceData, {
    ...
    if (error) {
        ...
        return json({ error: 'Failed to save workspace', details: error.message }, { status: 500, headers: responseHeaders });
    }
