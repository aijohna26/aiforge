  ...
  if (request.method === 'POST') {
    try {
      ...
      if (!image_url || !prompt || !model) {
        return json({ error: 'Missing required fields: image_url, prompt, model' }, { status: 400, headers });
      }
      ...
      return json({ error: 'Failed to save screen' }, { status: 500, headers });
    }
    ...
    try {
      ...
      if (!screenId) {
        return json({ error: 'Screen ID required' }, { status: 400, headers });
      }

      const { error } = await supabase.from('saved_screens').delete().eq('id', screenId).eq('user_id', user.id);
      ...
      console.error('[Screens API] Error:', error);
      return json({ error: 'Failed to delete screen' }, { status: 500, headers });
    }
  } else {
