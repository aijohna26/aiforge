...
export function BrandStyleFrame() {
    ...
    const generateLogoPrompt = useCallback(() => {
        ...
        const paletteLine = selectedPalette && colorPalette
        ...
        const textDirective = step3.logoTextMode === 'with-text'
    ...
    }, [step1.appName, step1.category, step1.description, step3.colorPalette, step3.logoTextMode, step3.paletteOptions, step3.selectedPaletteId, step3.selectedStyleId, step3.selectedTypographyId, step3.styleDirections, step3.typographyOptions]);
    ...
    const handleSelectLogo = useCallback((url: string) => {
    ...
    const hasResults =
        step3.paletteOptions.length || step3.typographyOptions.length || step3.styleDirections.length;
            ...
            <div className="w-[990px] max-h-[85vh] overflow-y-auto custom-scrollbar pointer-events-auto bg-[#11121D] border-2 border-[#1F243B] rounded-2xl p-10 pb-60 shadow-2xl text-white">
                ...
                {manualEntryActive && (
                    <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-500/60 bg-slate-800/70 px-4 py-3 text-sm text-white">
                        <p>
                            Manual style guide loaded. Review your color, typography, and style tabs before generating logos.
                        </p>
                        <button
                            onClick={() => setActiveTab('logo')}
                            disabled={activeTab === 'logo'}
                            className={`rounded-full px-4 py-1.5 text-xs font-semibold uppercase tracking-wide ${activeTab === 'logo'
                    ...
                    <>
                        ...
                        {activeTab === 'colors' && step3.paletteOptions.length > 0 && (
                            <div className="mt-8 grid grid-cols-2 gap-6">
                                {step3.paletteOptions.map((palette) => (
                                    <div
                                        key={palette.id}
                                        className={`rounded-2xl border p-6 bg-gradient-to-b from-[#171C2D] to-[#11121D] ${step3.selectedPaletteId === palette.id
                                    ...
                                    >
                                        <div className="flex justify-between items-start">
                                            ...
                                            <button
                                                ...
                                                className={`px-3 py-1.5 rounded-full text-xs font-semibold ${step3.selectedPaletteId === palette.id
                        ...
                        {activeTab === 'typography' && step3.typographyOptions.length > 0 && (
                            <div className="mt-8 grid grid-cols-2 gap-6">
                                {step3.typographyOptions.map((type) => (
                                    <div
                                        key={type.id}
                                        className={`rounded-2xl border p-6 bg-gradient-to-b from-[#171C2D] to-[#11121D] ${step3.selectedTypographyId === type.id
                                            ? 'border-blue-500 shadow-[0_0_25px_rgba(59,130,246,0.25)]'
                                            : 'border-[#1F243B]'
                                            }`}
                                    >
                                        <div className="flex items-start justify-between mb-4">
                                            ...
                                            <button
                                                ...
                                                className={`px-3 py-1.5 rounded-full text-xs font-semibold ${step3.selectedTypographyId === type.id
                                                    ? 'bg-blue-500/20 text-blue-200'
                                                    : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
                                                    }`}
                                            >
                                        ...
                                        <div className="rounded-xl border border-slate-700 bg-slate-900/40 p-4 text-sm space-y-3">
                                            <div>
                                                ...
                                                <p
                                                    style={{ fontFamily: `${type.headingFont}, 'Inter'` }}
                                            ...
                                            <div>
                                                ...
                                                <p
                                                    style={{ fontFamily: `${type.bodyFont}, 'Inter'` }}
                                                    className="text-base"
                                                >
                                                    {type.sampleText}
                                                </p>
                        ...
                        {activeTab === 'style' && step3.styleDirections.length > 0 && (
                            <div className="mt-8 grid grid-cols-2 gap-6">
                                {step3.styleDirections.map((style) => (
                                    <div
                                        key={style.id}
                                        className={`rounded-2xl border p-6 bg-gradient-to-b from-[#171C2D] to-[#11121D] ${step3.selectedStyleId === style.id
                                            ? 'border-blue-500 shadow-[0_0_25px_rgba(59,130,246,0.25)]'
                                            : 'border-[#1F243B]'
                                            }`}
                                    >
                                        <div className="flex justify-between items-start">
                                            ...
                                            <button
                                                ...
                                                className={`px-3 py-1.5 rounded-full text-xs font-semibold ${step3.selectedStyleId === style.id
                                                    ? 'bg-blue-500/20 text-blue-200'
                                                    : 'bg-slate-800 text-slate-300 hover:bg-slate-700'
                                                    }`}
                                            >
                                        ...
                                        <div className="mt-4 flex flex-wrap gap-2">
                                            {style.keywords.map((keyword) => (
                                                <span
                                                    key={keyword}
                                                    className="px-2.5 py-1 text-xs rounded-full bg-slate-800 text-slate-200"
                                                >
                                                    {keyword}
                                                </span>
                                            ))}
                        ...
                        {activeTab === 'logo' && (
                            <div className="mt-8 space-y-6">
                                {/* Prompt Input */}
                                <div>
                                    <label className="block text-sm font-bold text-slate-100 mb-2 uppercase tracking-wide">Logo Prompt</label>
                                    <textarea
                                        value={logoPrompt}
                                        onChange={(e) => {
                                            setPromptDirty(true);
                                ...
                                {logoUrls.length > 0 && (
                                    <div className="space-y-4">
                                        <h4 className="text-sm font-semibold text-slate-300">
                                            Generated Logos ({logoUrls.length})
                                        </h4>
                                        <div className="grid grid-cols-4 gap-4">
                                            {logoUrls.map((url, index) => (
                                                <div
                                                    ...
                                                    className={`relative rounded-xl border-2 overflow-hidden bg-slate-900 p-4 cursor-pointer transition-all group ${selectedLogo === url
                        ...
                        <div className="mt-8 text-slate-400 text-xs flex justify-between">
                            ...
                            {step3.lastExtractedAt && (
                                <div>Last analyzed {new Date(step3.lastExtractedAt).toLocaleTimeString()}</div>
                            )}
                        </div>
                    </>
                )}
