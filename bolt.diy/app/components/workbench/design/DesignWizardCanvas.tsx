...
export function DesignWizardCanvas({ zoom = 1, panX = 0, panY = 0, onRecenter }: DesignWizardCanvasProps) {
  ...
  const handleNext = async () => {
    if (!canProceedToNextStep() || isAnimating || currentStep >= 7 || isExtracting) {
      return;
    }

    if (awaitingLogoCompletion) {
      return;
    }
    ...
    if (currentStep === 2) {
      ...
      if (!wizardData.step2.referenceImages.length) {
        return;
      }
    ...
    try {
      ...
      const finalizedWizardData = {
        ...
        step3: {
          ...wizardData.step3,
          logo:
            wizardData.step3.logo && finalizeData.logoUrl
      ...
      const projectKey =
        finalizedWizardData.step7.projectName
          .toUpperCase()
          .replace(/[^A-Z0-9]/g, '')
          .substring(0, 4) || 'PROJ';
    ...
    <>
      ...
      >
        ...
        >
          ...
          <span className="text-slate-200 group-hover:text-red-400 font-medium text-sm transition-colors">
            Clear Session
          </span>
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-[#1a1a1a] border border-[#333] rounded-xl p-6 shadow-2xl max-w-md mx-4">
            ...
            <p className="text-slate-100 font-medium mb-6">
              Are you sure you want to clear all your progress? This will reset the entire design wizard and cannot be
              undone.
            </p>
      ...
      <div
        className={`absolute bottom-8 left-1/2 -translate-x-1/2 z-[9970] pointer-events-none transition-opacity duration-300 ${wizardData.step5.isStudioActive ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
      >
