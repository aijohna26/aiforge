...
export function Step5Interactive() {
  ...
  const handleRegenerateTheme = useCallback(async () => {
    ...
    try {
      const selectedNav = wizardData.step4.navigation.navBarVariations.find(
        (v) => v.id === wizardData.step4.navigation.selectedVariationId,
      );
  ...
  useScreenGenerationPolling(jobId, (result) => {
    ...
    if (result?.screens) {
      ...
      setFrames((prev) => {
        ...
        console.log(
          '[Inngest] Current frames before update:',
          prev.map((f) => ({ id: f.id, hasHtml: !!f.html })),
        );
        ...
        result.screens.forEach((screen: any, index: number) => {
          ...
          if (existingIndex === -1 && index < updatedFrames.length && !updatedFrames[index].html) {
            console.warn(
              `[Inngest] ID mismatch for screen index ${index}. Expected ${updatedFrames[index].id}, got ${screen.id}. Updating by index.`,
            );
            existingIndex = index;
          }
          ...
          if (existingIndex >= 0) {
            ...
            console.log(
              `[Inngest] Updating frame ${updatedFrames[existingIndex].id} with content (length: ${screen.html?.length})`,
            );
  ...
  const persistStudioFrames = useCallback(
    (nextFrames: FrameData[], nextTheme?: any) => {
      updateStep5Data({
  ...
  const handleSave = useCallback(async () => {
    ...
    if (frames.length === 0 || isSavingRef.current) {
      return;
    }
    ...
    (frameId: string) => {
      setFrames((prev) => {
        ...
        const isOccupied = (x: number) =>
          prev.some((frame) => Math.abs((frame.x ?? 0) - x) < 10 && (frame.y ?? 0) === baseY);
          ...
          >
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-4">
                <h2 className="text-base font-black text-white/95 tracking-[-0.01em]">
                  {wizardData.step1.appName || 'Untitled App'}
                </h2>
      ...
      <AnimatePresence>
        {status === 'generating' && jobId && (
          ...
          >
            ...
            <JobProgressBar jobId={jobId} showDetails={true} />
          </motion.div>
        )}
      </AnimatePresence>
            ...
            >
              ...
              <div className="relative w-full h-full flex items-center justify-center p-12 group">
                ...
                <div className="absolute inset-0 overflow-hidden">
                  ...
                  <div
                    className="absolute bottom-1/4 right-1/4 size-96 bg-emerald-600/10 rounded-full blur-[100px] animate-pulse"
