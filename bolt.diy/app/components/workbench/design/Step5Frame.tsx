...
const SCREEN_TYPE_GUIDANCE: Record<string, string> = {
  splash:
    'Pure splash screen hero moment: Place the primary reference image logo in the center with the app name and a short tagline underneath. Atmosphere: Premium, spacious, and minimalist. Background should be a subtle gradient or branded texture. ABSOLUTELY NO buttons, cards, or interactive UI elements.',
  signin:
    'Authentication screen. Hierarchy: Place the primary reference image logo at the top (prominent), followed by a clear "Sign In" heading. Input fields for Email and Password should be clean and modern with clear labels. Single primary CTA button. Social login buttons (Apple/Google) should be at the bottom, separate from the primary form.',
  signup:
    'Registration screen. Hierarchy: Place the primary reference image logo at the top, clear "Create Account" heading. Form fields with consistent spacing. Primary CTA button should be the most prominent element.',
  ...
  profile:
    'User profile screen. Hierarchy: Top section with user avatar, name, and bio. Followed by editable menu categories, user stats (e.g. followers, activity), and a prominent logout or settings action.',
  settings:
    'Settings hub. Hierarchy: Categorized list items with clean icons and descriptive labels. Use switches, toggles, and disclosure indicators (chevron-right). Minimalist design with strong vertical rhythm.',
  custom:
    'High-fidelity mobile UI screen. Hierarchy: Dedicated header section, followed by structured content cards and interactive components. Maintain consistent padding, premium typography, and clear visual hierarchy.',
};
...
const getOriginalUrl = (url: string | null | undefined) => {
  ...
  if (url.startsWith('data:') || url.startsWith('blob:')) {
    console.error(
      '[Step5] Data URLs are not supported for reference images. URL must be HTTP/HTTPS:',
      url.substring(0, 100),
    );
    return null;
  }
...
export function Step5Frame() {
  ...
  const [screenToManualEdit, setScreenToManualEdit] = useState<{ id: string; url: string; variationId: string } | null>(
    null,
  );
  ...
  const navigationState = step4.navigation ?? {
    type: 'bottom',
    items: [],
    confirmed: false,
  ...
  const getIncludeNav = useCallback(
    (screenId: string) => {
  ...
  const toggleIncludeNav = useCallback(
    (screenId: string) => {
  ...
  const getIncludeLogo = useCallback(
    (screenId: string) => {
  ...
  const toggleIncludeLogo = useCallback(
    (screenId: string) => {
  ...
  const buildScreenPrompt = useCallback(
    (screen: (typeof screens)[number], includeNavOverride?: boolean, includeLogoOverride?: boolean) => {
      ...
      const keyElements =
        screen.keyElements && screen.keyElements.length ? `Key elements: ${screen.keyElements.join(', ')}.` : '';
      const palette = step3.colorPalette
        ...
        : '';
      const typography = step3.typography ? `Typography pairing: ${step3.typography.fontFamily}.` : '';
      ...
      const styleKeywords = styleDirection?.keywords?.length
        ...
        : '';
      const personality = step2.personality ? `Overall mood should feel ${step2.personality}.` : '';
      const componentTone =
      ...
      const typeGuidance = SCREEN_TYPE_GUIDANCE[screen.type] || SCREEN_TYPE_GUIDANCE.custom;
      ...
      const globalConstraints =
        'IMPORTANT: Render ONLY the UI content. DO NOT include phone frames, device bezels, notches, or OS status bars (time, battery, signal). The design should fill the entire 9:16 canvas. Use the provided reference images as the absolute truth for visual style, branding, and layouts.';
  ...
  const handleGenerateScreen = useCallback(
    async (screenId: string, quantityOverride?: number) => {
      ...
      if (isGemini) {
        ...
        if (screenHasRefs && googleModel === 'nano-banana') {
          toast.error(
            `"${screen.name}" has branding enabled (Nav/Logo), but Gemini Nano Banana Standard does not support it. Please switch to Gemini Nano Banana Pro.`,
          );
          return;
        }
        ...
        if (!screenHasRefs && googleModel === 'nano-banana-edit') {
          toast.error(
            `"${screen.name}" does not have branding enabled, but Gemini Nano Banana Edit requires reference images. Please switch to Standard or Pro.`,
          );
          return;
        }
      }
      ...
      try {
        ...
        for (let i = 0; i < quantity; i++) {
          ...
          if (hasInvalidReferences) {
            throw new Error(
              'Reference images must be uploaded to a server first. Data URLs are not supported by the image generation API. Please regenerate your logo or navigation bar.',
            );
          }
        ...
        const creditDelta = newVariations.reduce((sum, variation) => sum + (variation.creditsUsed || 0), 0);
        ...
        const description = 'The design engine may be busy';

        if (error instanceof Error) {
          // Check for insufficient credits error (402 status or message contains "insufficient credits")
          const isCreditError =
            (error as any)?.status === 402 || error.message.toLowerCase().includes('insufficient credits');
          ...
          if (isCreditError) {
            ...
            showAlert({
              ...
              description:
                error.message ||
                'You need more credits to generate this screen. Please add credits or choose a cheaper model.',
            });

            return; // Exit early
          } else if (
            error.message.includes('Image Service Error') ||
    ...
    [
      ...
      buildScreenPrompt,
      splashLogoUrl,
      navigationState.confirmed,
      navigationType,
      getIncludeNav,
      getIncludeLogo,
      hasNavigationBar,
      navigationState.generatedNavBar,
    ],
  );

  const handleGenerateAll = useCallback(async () => {
    if (!screensMissingGeneration.length) {
      return;
    }
  ...
  const handleDeleteVariation = useCallback(
    (screenId: string, variationId: string) => {
      ...
      const nextSelected = remaining.find((variation) => variation.id === target.selectedVariationId) || remaining[0];
  ...
  const handleEditScreen = useCallback(async () => {
    if (!screenToEdit || !editPrompt.trim()) {
      return;
    }
    ...
    try {
      ...
      const providerName =
        modelConfig?.provider === 'gemini'
          ? 'Gemini'
          : modelConfig?.provider === 'seedream-4.5-edit'
            ? 'Seedream'
            : modelConfig?.provider === 'gpt-image-1'
              ? 'OpenAI'
              : 'Qwen';
      ...
      const updatedScreens = step5.generatedScreens.map((gen) => {
        if (gen.screenId !== screenToEdit.id) {
          return gen;
        }
      ...
      if (error instanceof Error) {
        ...
        if (isCreditError) {
          ...
          showAlert({
            ...
            description:
              error.message ||
              'You need more credits to edit this screen. Please add credits or choose a cheaper model.',
          });
  ...
  const handleSaveManualEdit = useCallback(
    async (editedImageUrl: string) => {
      ...
      const updatedScreens = step5.generatedScreens.map((gen) => {
        if (gen.screenId !== screenToManualEdit.id) {
          return gen;
        }
      ...
      <div className="w-[1210px] max-h-[85vh] overflow-y-auto custom-scrollbar pointer-events-auto bg-[#11121D] border-2 border-[#1F243B] rounded-2xl p-10 pb-60 shadow-2xl text-white">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-100 font-medium">Generate multi-variation designs for each key screen</p>
          </div>
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-slate-100 leading-tight font-medium">
                  {splashLogoUrl
                    ? 'Configured in Step 3. Used for splash screens & branding.'
                    : 'No logo selected. Add one in Step 3.'}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-5">
              <div className="relative w-20 h-20 rounded-xl bg-slate-900 border border-[#2F344B] group-hover:border-emerald-500/40 flex items-center justify-center overflow-hidden transition-all duration-300">
                {hasNavigationBar && navigationState.generatedNavBar ? (
                  <img
                    src={navigationState.generatedNavBar.url}
                    alt="Nav Bar"
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-slate-100 leading-tight font-medium">
                  {hasNavigationBar
                    ? 'Selected in Step 4. Can be toggled on specific screens.'
                    : 'No navigation bar selected in Step 4.'}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8 bg-[#171C2D]/50 border border-[#1F243B] rounded-2xl p-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                provider === 'gemini' ? setGoogleModel(e.target.value) : setOpenaiModel(e.target.value)
              }
              className="w-full px-3 py-2 rounded-lg bg-[#0B0F1C] border border-[#1F243B] focus:border-blue-500 outline-none text-sm transition-all"
            >
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1.5 text-ellipsis overflow-hidden">
                          <h4
                            className={`font-semibold transition-colors ${isActive ? 'text-white' : 'text-slate-200'}`}
                          >
                            {screen.name}
                          </h4>
                    ...
                    <>
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          ...
                          <p className="text-xs text-slate-400">{selectedScreen.purpose || 'Core experience screen'}</p>
                        </div>
                        <button
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-xs text-slate-400">Variations ({selectedGenerated.variations.length})</p>
                        <button
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-4">
                          {selectedGenerated.variations.map((variation) => {
                            ...
                            return (
                              <div
                                key={variation.id}
                                className={`relative rounded border bg-slate-900 aspect-[9/16] overflow-hidden cursor-pointer group ${
                                  isActive ? 'border-blue-500' : 'border-[#1F243B]'
                                }`}
                                onClick={() => handleSelectVariation(selectedGenerated.screenId, variation.id)}
                              >
                                <img src={variation.url} alt="Variation" className="w-full h-full object-cover" />
                      ...
                      <div className="space-y-2 text-xs text-slate-400 border-t border-slate-700 pt-3">
                        ...
                        <div className="pt-2">
                          ...
                          <p className="text-white text-xs leading-relaxed">{selectedGenerated.prompt}</p>
                        </div>
                      </div>
                    </>
                  )
        ...
        {step5.generatedScreens.length > 0 && (
          <div className="mt-6 pt-6 border-t border-slate-700 flex items-center justify-between text-sm">
            <div className="text-slate-400">
              Generated: <strong className="text-white">{step5.generatedScreens.length}</strong> of{' '}
              <strong className="text-white">{screens.length}</strong> screens
            </div>
            <div className="text-slate-400">
              Selected:{' '}
              <strong className="text-green-400">{step5.generatedScreens.filter((g) => g.selected).length}</strong>{' '}
              screens for app
            </div>
          </div>
        )}
            ...
            >
              ...
              >
                ...
                <p className="text-sm text-slate-300 mb-6">
                  This will remove all generated mockups for this screen. You can regenerate it later if needed.
                </p>
              ...
              >
                ...
                <div className="grid gap-5 mb-6 md:grid-cols-[minmax(180px,240px)_1fr] items-start">
                  ...
                  <div className="flex-1 flex flex-col space-y-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Edit Model</label>
                      <select
                        value={editModel}
