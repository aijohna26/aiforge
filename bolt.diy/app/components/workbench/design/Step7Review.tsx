            ...
            <div className="w-full space-y-4">
              <div>
                <h4 className="text-2xl font-black text-white leading-tight">
                  {step7.projectName || step1.appName || 'Untitled Project'}
                </h4>
                <div>
                  <p
                    className={`text-slate-400 text-sm mt-1 leading-relaxed font-medium ${!isDescriptionExpanded ? 'line-clamp-2' : ''}`}
                  >
                    {step1.description}
                  </p>
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  <span className="text-[10px] text-slate-400 bg-slate-800/50 px-1.5 py-0.5 rounded">
                    H1: {step3.typography?.scale.h1.size}px
                  </span>
                  <span className="text-[10px] text-slate-400 bg-slate-800/50 px-1.5 py-0.5 rounded">
                    Body: {step3.typography?.scale.body.size}px
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar">
            {selectedScreens.map((screen: any, idx: number) => (
              <div key={idx} className="flex-shrink-0 w-36 group/screen">
                ...
                <p className="text-[10px] font-black text-white mt-2 truncate uppercase tracking-tighter">
                  {screen.name}
                </p>
              </div>
            ))}
                    ...
                    {enabledIntegrations.length > 0 ? (
                      enabledIntegrations.map((integration: any) => (
                        ...
                        >
                          ...
                          <span className="text-[11px] font-black text-green-100 uppercase tracking-tight">
                            {integration.id}
                          </span>
                        </div>
                      ))
                      ...
                      step6.dataModels.map((model: any) => (
                        <div key={model.id} className="group relative">
                          <div className="flex items-center gap-2 px-3 py-1.5 bg-cyan-500/10 border border-cyan-500/30 rounded-lg">
                            ...
                            <span className="text-[11px] font-black text-cyan-100 uppercase tracking-tight">
                              {model.name}
                            </span>
                          </div>
                          {/* Tooltip on hover */}
                          <div className="absolute bottom-full left-0 mb-2 w-48 p-2 bg-[#1A2133] border border-cyan-500/30 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 shadow-2xl">
                            <p className="text-[9px] text-cyan-400 font-black uppercase mb-1">
...
function Badge({ icon, label, color }: { icon: string; label: string; color: 'blue' | 'purple' | 'amber' | 'rose' }) {
  ...
  return (
    <div
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border ${colors[color]} text-xs font-black uppercase tracking-tight`}
    >
...
function ColorDisk({ color, label }: { color: string; label: string }) {
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div className="w-8 h-8 rounded-full border-2 border-[#1E2533] shadow-lg" style={{ backgroundColor: color }} />
