...
export function ImageEditor({ imageUrl, onSave, onCancel }: ImageEditorProps) {
  ...
  const drawImageOnCanvas = useCallback(
    (img: HTMLImageElement, cropRect?: CropState) => {
  ...
  const handleCanvasMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    if (!isDragging || activeTool !== 'crop' || !canvasRef.current) {
      return;
    }
  ...
  const handleApplyCrop = () => {
    if (!currentImage || !canvasRef.current || cropState.width === 0 || cropState.height === 0) {
      return;
    }
    ...
    ctx.drawImage(currentImage, actualX, actualY, actualWidth, actualHeight, 0, 0, actualWidth, actualHeight);
      ...
      <div className="bg-[#0b0c14] border border-[#1F243B] rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-all ${
                activeTool === 'crop' ? 'bg-blue-600 text-white' : 'bg-[#171C2D] text-slate-300 hover:bg-[#1F243B]'
              }`}
              disabled={isProcessing || isUploading}
            >
              <div className="flex items-center gap-2">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path
                    ...
                    strokeWidth={2}
                    d="M7 7h10v10M7 7V3M7 7H3m14 14h-4m0 0v-4m0 4H7"
                  />
                </svg>
                Crop
              </div>
        ...
        <div className="flex-1 overflow-auto p-6 flex items-center justify-center bg-[#0D1117]">
          ...
          <canvas
            ...
            className={`border border-[#2F344B] rounded-lg ${activeTool === 'crop' ? 'cursor-crosshair' : ''}`}
