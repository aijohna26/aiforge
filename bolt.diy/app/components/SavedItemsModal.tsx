...
export function SavedItemsModal({ isOpen, onClose, savedLogo, savedScreens, onRemoveItem, onUpdateItem }: SavedItemsModalProps) {
        ...
        >
            ...
            >
                ...
                <div className="flex-1 flex overflow-hidden">
                    {/* Main Viewer Area */}
                    <div className={`flex-1 relative flex flex-col p-8 transition-all duration-500 ${isEditingAI ? 'bg-black/40' : ''}`}>
                        {!hasItems ? (
                            ...
                            <>
                                ...
                                <div className="flex justify-between items-center mb-8">
                                    <div className="flex items-center gap-3">
                                        <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border transition-colors ${currentItem.type === 'logo'
                                ...
                                <div className="flex-1 flex items-center justify-center relative">
                                    <div className="relative group max-w-full h-full flex items-center justify-center">
                                        <div className={`absolute inset-0 bg-indigo-500/10 blur-[100px] rounded-full transition-opacity duration-700 ${isGenerating ? 'opacity-100 animate-pulse' : 'opacity-20'}`} />
                                        ...
                                        {isGenerating && (
                                            <div className="absolute inset-0 flex items-center justify-center">
                                                <div className="flex flex-col items-center gap-4">
                                                    ...
                                                    <span className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] animate-pulse">Processing...</span>
                                                </div>
                                            </div>
                                        )}
                        ...
                        <div className="w-[380px] border-l border-[#1F243B] bg-[#0d0e1a]/80 backdrop-blur-3xl flex flex-col p-8 animate-in slide-in-from-right duration-500">
                            ...
                            <div className="space-y-6 flex-1 overflow-y-auto custom-scrollbar pr-2">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-white/40 uppercase tracking-[0.15em]">Instructions</label>
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-white/40 uppercase tracking-[0.15em]">Intelligence Engine</label>
                                    <div className="grid grid-cols-1 gap-2">
                                        {EDIT_MODELS.map((model) => (
                                            <button
                                                ...
                                                disabled={isGenerating}
                                                className={`flex items-center justify-between p-4 rounded-xl border transition-all ${editModel === model.value
                                                        ? 'bg-indigo-600/10 border-indigo-500/40 text-white'
                                                        : 'bg-white/[0.02] border-white/[0.05] text-white/40 hover:bg-white/[0.04]'
                                                    }`}
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className={`${model.icon} text-lg ${editModel === model.value ? 'text-indigo-400' : 'opacity-40'}`} />
                ...
                {allItems.length > 0 && (
                    <div className="p-6 bg-black/40 border-t border-[#1F243B]">
                        <div className="flex items-center justify-between mb-4">
                            <span className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em]">Repository Explorer</span>
                            <span className="text-[10px] font-black text-white/30 font-mono tracking-widest">{currentIndex + 1} / {allItems.length}</span>
                        </div>
                        <div className="flex gap-3 overflow-x-auto custom-scrollbar pb-2">
                            {allItems.map((item, index) => (
                                <button
                                    ...
                                    className={`relative flex-shrink-0 size-20 rounded-xl overflow-hidden border-2 transition-all group ${index === currentIndex
            ...
            {isManualEditing && currentItem && typeof document !== 'undefined' && createPortal(
                <ImageEditor
