...
class WalletManager {
  ...
  async reserve(userId: string, amount: number): Promise<boolean> {
    if (supabase) {
      ...
      const { data, error } = await supabase.from('wallets').select('balance, reserved').eq('user_id', userId).single();
  ...
  async settle(userId: string, reservedAmount: number, actualCost: number): Promise<void> {
    if (supabase) {
      const { data } = await supabase.from('wallets').select('balance, reserved').eq('user_id', userId).single();
  ...
  async resetReserved(userId: string): Promise<void> {
    if (supabase) {
      await supabase.from('wallets').update({ reserved: 0 }).eq('user_id', userId);
      return;
    }
  ...
  private async ensureWallet(userId: string): Promise<void> {
    ...
    const { data } = await supabase.from('wallets').select('user_id').eq('user_id', userId).single();
