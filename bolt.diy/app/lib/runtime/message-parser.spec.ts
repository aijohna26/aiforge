...
describe('StreamingMessageParser', () => {
  ...
  describe('no artifacts', () => {
    ...
    ])('should correctly parse chunks and strip out af artifacts (%#)', (input, expected) => {
  ...
  describe('invalid or incomplete artifacts', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      ...
      ['Foo bar <bol', 'Foo bar '],
      ['Foo bar <af', 'Foo bar '],
      ['Foo bar <afa', 'Foo bar <afa'],
      ['Foo bar <afA', 'Foo bar '],
      ['Foo bar <afArtifacs></afArtifact>', 'Foo bar <afArtifacs></afArtifact>'],
      ['Before <oltArtfiact>foo</afArtifact> After', 'Before <oltArtfiact>foo</afArtifact> After'],
      ['Before <afArtifactt>foo</afArtifact> After', 'Before <afArtifactt>foo</afArtifact> After'],
    ])('should correctly parse chunks and strip out af artifacts (%#)', (input, expected) => {
      runTest(input, expected);
    });
  });
  ...
  describe('valid artifacts without actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Some text before <afArtifact title="Some title" id="artifact_1">foo bar</afArtifact> Some more text',
        {
      ...
      [
        [
          'Some text before <afArti',
          'fact',
          ' title="Some title" id="artifact_1" type="bundled" >foo</afArtifact> Some more text',
        ],
        {
          output: 'Some text before  Some more text',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
      ...
      [
        'Before <afArtifact title="Some title" id="artifact_1">foo</afArtifact> After',
        {
          output: 'Before  After',
          callbacks: { onArtifactOpen: 1, onArtifactClose: 1, onActionOpen: 0, onActionClose: 0 },
        },
  ...
  describe('valid artifacts with actions', () => {
    it.each<[string | string[], ExpectedResult | string]>([
      [
        'Before <afArtifact title="Some title" id="artifact_1"><afAction type="shell">npm install</afAction></afArtifact> After',
        {
          output: 'Before  After',
      ...
      [
        'Before <afArtifact title="Some title" id="artifact_1"><afAction type="shell">npm install</afAction><afAction type="file" filePath="index.js">some content</afAction></afArtifact> After',
        {
          output: 'Before  After',
