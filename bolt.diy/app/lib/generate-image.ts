...
class ImageGenerationService {
  ...
  private async generateWithKie4o(options: ImageGenerationOptions): Promise<ImageGenerationResult> {
    ...
    if (!createResponse.ok) {
      ...
      if (statusCode === 500) {
        throw new Error(
          `Image generation service error (500): ${errorMessage}. This usually means the service is temporarily overloaded. Please wait a moment and try again. If the issue persists, try simplifying your prompt or using a different model.`,
        );
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      const queryResponse = await fetch(`https://api.kie.ai/api/v1/gpt4o-image/record-info?taskId=${taskId}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${apiKey}`,
        },
    ...
    throw new Error(
      `Image generation timed out after ${(maxAttempts * pollInterval) / 1000} seconds. The task may still be processing. Please try again in a few moments, or try a simpler prompt.`,
    );
  }
  ...
  private async generateWithKie(options: ImageGenerationOptions): Promise<ImageGenerationResult> {
    ...
    if (model === 'nano-banana-edit') {
      ...
      if (!options.referenceImages || options.referenceImages.length === 0) {
        throw new Error(
          'nano-banana-edit requires at least one reference image for editing. Please provide a reference image or use nano-banana for generation from text only.',
        );
      }
      ...
      if (model === 'nano-banana') {
        console.warn(
          '[Image Service] Reference images are not supported by nano-banana. Proceeding without references.',
        );
      ...
      input: {
        ...
        ...(options.referenceImages &&
          options.referenceImages.length > 0 && {
            [model === 'nano-banana-edit' ? 'image_urls' : 'image_input']:
    ...
    console.log(
      `[Image Service] Creating task for model: ${modelName} with ${options.referenceImages?.length || 0} reference images`,
    );
    ...
    if (options.referenceImages && options.referenceImages.length > 0) {
      options.referenceImages.forEach((img, idx) => {
        if (img.startsWith('data:')) {
          console.warn(
            `[Image Service] Reference image ${idx} is a DATA URL (base64). This will likely fail with Kie.ai!`,
          );
    ...
    if (!createResponse.ok) {
      ...
      if (statusCode === 500) {
        ...
        if (errorMessage.includes('image_urls is required') || errorMessage.includes('image_urls')) {
          throw new Error(
            `Image generation error: The selected model requires reference images. If you're trying to edit an image, please provide a reference image. If you're generating from text only, use 'nano-banana' model instead of 'nano-banana-edit'.`,
          );
        }

        throw new Error(
          `Image generation service error (500): ${errorMessage}. The service is temporarily overloaded. Please wait a moment and try again. For complex requests with icons and text, try using GPT-Image-1 model instead.`,
        );
      } else if (statusCode === 429) {
        throw new Error(`Rate limit exceeded (429): ${errorMessage}. Please wait a few moments before trying again.`);
      } else if (statusCode === 400) {
    ...
    throw new Error(
      `Image generation timed out after ${(maxAttempts * pollInterval) / 1000} seconds using model ${model}. The task may still be processing. Please try again in a few moments, or try a simpler prompt. For complex requests with icons and text, consider using GPT-Image-1 model which handles text better.`,
    );
  }
  ...
  private enhancePromptForUI(originalPrompt: string): string {
    ...
    const EDGE_RULES =
      'FULL-BLEED DESIGN: The background MUST touch all 4 square edges of the image file. NO white borders, NO rounded screen corners, NO white gaps. IGNORE any borders in the reference image and bleed the background to the absolute edges.';
