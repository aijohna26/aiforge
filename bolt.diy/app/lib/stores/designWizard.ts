...
export interface Step4Data {
  screens: Array<{
    id: string;
    name: string;
    type: 'splash' | 'signin' | 'signup' | 'home' | 'profile' | 'settings' | 'custom' | 'scanner' | 'onboarding';
    purpose: string;
    keyElements: string[];
...
const migrateStoredWizardData = (data: DesignWizardData): DesignWizardData => {
  ...
  if (migrated.step6?.integrations) {
    migrated.step6.integrations = migrated.step6.integrations.filter((i) => !['convex', 'convex-auth'].includes(i.id));
  }
  ...
  if (!migrated.step5) {
    ...
    if (!migrated.step5.studioFrames) {
      migrated.step5.studioFrames = [];
    }

    if (!migrated.step5.generatedScreens) {
      migrated.step5.generatedScreens = [];
    }

    if (migrated.step5.isStudioActive === undefined) {
      migrated.step5.isStudioActive = false;
    }

    if (migrated.step5.customTheme === undefined) {
      migrated.step5.customTheme = null;
    }
  }
...
export function isStep1Complete(): boolean {
  ...
  return !!(step1.appName.trim() && step1.description.trim());
}
...
export function addReferenceImage(image: { id: string; url: string; file?: File; source: 'upload' | 'paste' }) {
  const current = designWizardStore.get();
