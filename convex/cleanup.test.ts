...
async function createChatId(t: TestConvex) {
  return await t.run(async (ctx: { db: { insert: (arg0: string, arg1: { creatorId?: any; initialId?: string; timestamp?: string; lastSubchatIndex?: number; }) => any; }; }) => {
...
describe("cleanup", () => {
  ...
  async function setupTestData() {
    ...
    const storageId = await t.run(async (ctx: { storage: { store: (arg0: Blob) => any; }; }) => {
      return await ctx.storage.store(new Blob(["test content"]));
    });

    // Create a chatMessagesStorageState
    const storageStateId = await t.run(async (ctx: { db: { insert: (arg0: string, arg1: { chatId: any; storageId: any; lastMessageRank: number; partIndex: number; subchatIndex: number; }) => any; }; }) => {
    ...
    const logId = await t.run(async (ctx: { db: { insert: (arg0: string, arg1: { chatId: any; subchatIndex: number; responseCoreMessages: never[]; promptCoreMessagesStorageId: any; finishReason: string; modelId: string; usage: { completionTokens: number; promptTokens: number; cachedPromptTokens: number; }; chefTokens: number; }) => any; }; }) => {
  ...
  test("deleteDebugFilesForInactiveChats only deletes from debugChatApiRequestLog and referenced storage", async () => {
    ...
    const logEntry = await t.run(async (ctx: { db: { get: (arg0: any) => any; }; }) => {
      return await ctx.db.get(logId);
    });
    expect(logEntry).toBeNull();

    // Verify the storage file was deleted
    const storageFile = await t.run(async (ctx: { storage: { getUrl: (arg0: any) => any; }; }) => {
      return await ctx.storage.getUrl(storageId);
    });
    expect(storageFile).toBeNull();

    // Verify the chat and storage state were NOT deleted
    const chat = await t.run(async (ctx: { db: { get: (arg0: any) => any; }; }) => {
      return await ctx.db.get(chatId);
    });
    expect(chat).not.toBeNull();

    const storageState = await t.run(async (ctx: { db: { get: (arg0: any) => any; }; }) => {
  ...
  test("no deletion when data is too recent", async () => {
    ...
    const scheduledJobs = await t.run(async (ctx: { db: { system: { query: (arg0: string) => { (): any; new(): any; collect: { (): any; new(): any; }; }; }; }; }) => {
...
describe("deleteOldStorageStatesForLastMessageRank", () => {
  ...
  test("deletes all storage states except the latest one", async () => {
    ...
    const storageStates = await t.run(async (ctx: { storage: { store: (arg0: Blob) => any; }; db: { insert: (arg0: string, arg1: { chatId: any; storageId: any; lastMessageRank: number; partIndex: number; subchatIndex: number; snapshotId: any; }) => any; }; }) => {
      const states = [];
      for (let i = 0; i < 3; i++) {
    ...
    const remainingStates = await t.run(async (ctx: { db: { query: (arg0: string) => { (): any; new(): any; withIndex: { (arg0: string, arg1: (q: any) => any): { (): any; new(): any; collect: { (): any; new(): any; }; }; new(): any; }; }; }; }) => {
      return await ctx.db
        .query("chatMessagesStorageState")
        .withIndex("byChatId", (q: { eq: (arg0: string, arg1: any) => any; }) => q.eq("chatId", chatId))
        .collect();
    });
    ...
    const snapshotFile = await t.run(async (ctx: { storage: { getUrl: (arg0: any) => any; }; }) => {
    ...
    await t.run(async (ctx: { db: { get: (arg0: any) => any; }; storage: { getUrl: (arg0: any) => any; }; }) => {
  ...
  test("does not delete snapshots that are referenced in other storage states", async () => {
    ...
    const chatId = await createChatId(t);
    // Create a snapshot
    const snapshotId = await t.run(async (ctx: { storage: { store: (arg0: Blob) => any; }; }) => {
      return await ctx.storage.store(new Blob(["content"]));
    });
    const snapshotId2 = await t.run(async (ctx: { storage: { store: (arg0: Blob) => any; }; }) => {
      return await ctx.storage.store(new Blob([`snapshot-2`]));
    });
    const snapshotId3 = await t.run(async (ctx: { storage: { store: (arg0: Blob) => any; }; }) => {
      return await ctx.storage.store(new Blob([`snapshot-3`]));
    });

    // Create multiple storage states for the different lastMessageRanks

    await t.run(async (ctx: { storage: { store: (arg0: Blob) => any; }; db: { insert: (arg0: string, arg1: { chatId: any; storageId: any; lastMessageRank: number; partIndex: number; subchatIndex: number; snapshotId: any; }) => any; }; }) => {
    ...
    const snapshotFile2 = await t.run(async (ctx: { storage: { getUrl: (arg0: any) => any; }; }) => {
      return await ctx.storage.getUrl(snapshotId2);
    });
    expect(snapshotFile2).not.toBeNull();
    // Verify the third snapshot is still there
    const snapshotFile3 = await t.run(async (ctx: { storage: { getUrl: (arg0: any) => any; }; }) => {
  ...
  test("does nothing when there is only one storage state", async () => {
    ...
    const stateId = await t.run(async (ctx: { db: { insert: (arg0: string, arg1: { chatId: any; storageId: any; lastMessageRank: number; partIndex: number; subchatIndex: number; snapshotId: any; }) => any; }; }) => {
      return await ctx.db.insert("chatMessagesStorageState", {
        chatId,
        storageId,
        lastMessageRank: 1,
    ...
    const state = await t.run(async (ctx: { db: { get: (arg0: any) => any; }; }) => {
  ...
  test("dry run does not delete anything", async () => {
    ...
    const storageFiles = await t.run(async (ctx: { db: { get: (arg0: any) => any; }; storage: { getUrl: (arg0: any) => any; }; }) => {
      const files = [];
...
describe("file cleanup tests", () => {
  ...
  test("deletes file when not referenced anywhere", async () => {
    ...
    // Run the cleanup function
    const result = await t.run(async (ctx: { db: { system: { get: (arg0: any) => any; }; }; }) => {
  ...
  test("does not delete file when referenced by chat", async () => {
    ...
    await t.run(async (ctx: { db: { insert: (arg0: string, arg1: { creatorId?: any; initialId?: string; timestamp?: string; lastSubchatIndex?: number; snapshotId?: any; }) => any; }; }) => {
  ...
  test("does not delete file when referenced by chatMessagesStorageState as snapshotId", async () => {
    ...
    const chatId = await createChatId(t);
    await t.run(async (ctx: { db: { insert: (arg0: string, arg1: { chatId: any; storageId: null; lastMessageRank: number; partIndex: number; subchatIndex: number; snapshotId: any; }) => any; }; }) => {
      await ctx.db.insert("chatMessagesStorageState", {
        chatId,
        storageId: null,
        lastMessageRank: 1,
  ...
  test("does not delete file when referenced by chatMessagesStorageState as storageId", async () => {
    ...
    // Create a chat and storage state that references the storage file
    const chatId = await createChatId(t);
    await t.run(async (ctx: { db: { insert: (arg0: string, arg1: { chatId: any; storageId: any; lastMessageRank: number; partIndex: number; subchatIndex: number; }) => any; }; }) => {
      await ctx.db.insert("chatMessagesStorageState", {
        chatId,
        storageId,
        lastMessageRank: 1,
  ...
  test("does not delete files when referenced by share", async () => {
    ...
    await t.run(async (ctx: { db: { query: (arg0: string) => { (): any; new(): any; collect: { (): any; new(): any; }; }; delete: (arg0: any) => any; }; }) => {
    ...
    const files = await t.run(async (ctx: { db: { system: { query: (arg0: string) => { (): any; new(): any; collect: { (): any; new(): any; }; }; }; }; }) => await ctx.db.system.query("_storage").collect());
  ...
  test("does not delete file when referenced by socialShare", async () => {
    ...
    await t.run(async (ctx: { db: { patch: (arg0: any, arg1: { thumbnailImageStorageId: any; }) => any; }; }) => {
  ...
  test("does not delete file when referenced by debugChatApiRequestLog", async () => {
    ...
    const chatId = await createChatId(t);
    await t.run(async (ctx: { db: { insert: (arg0: string, arg1: { chatId: any; subchatIndex: number; responseCoreMessages: never[]; promptCoreMessagesStorageId: any; finishReason: string; modelId: string; usage: { completionTokens: number; promptTokens: number; cachedPromptTokens: number; }; chefTokens: number; }) => any; }; }) => {
