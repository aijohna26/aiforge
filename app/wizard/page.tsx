        ...
        if (saved) {
            ...
            if (parsed.appInfo && !parsed.appInfo.brandColors) {
                ...
                if (parsed.appInfo.colorScheme) {
                    const colorMatch = parsed.appInfo.colorScheme.match(/Primary:\s*(#[0-9A-Fa-f]{6})[,)].*Secondary:\s*(#[0-9A-Fa-f]{6})[,)].*Accent:\s*(#[0-9A-Fa-f]{6})/);
                    if (colorMatch) {
                        parsed.appInfo.brandColors = {
...
export default function WizardPage() {
    ...
    // Ensure step 2 always has a palette selected so navigation isn't blocked
    useEffect(() => {
        if (!isLoaded || currentStep !== 2 || appInfo.colorScheme) return;
        const defaultScheme = colorSchemes[0];
        if (!defaultScheme) return;
        setAppInfo((prev) => ({
            ...prev,
            colorScheme: defaultScheme.value,
            brandColors: {
                primary: defaultScheme.colors[0],
                secondary: defaultScheme.colors[1],
                accent: defaultScheme.colors[2],
            },
        }));
    }, [isLoaded, currentStep, appInfo.colorScheme, setAppInfo]);
    ...
    const addReference = (url: string) => {
        if (!url.trim()) return;
        setStyleBoard((prev) => {
            if (prev.references.includes(url.trim())) return prev;
    ...
    const handleAddPackage = () => {
        ...
        setAppInfo((prev) => ({
            ...prev,
            selectedPackage: newPackage.id,
            aiConfig: newPackage.supportsAI ? prev.aiConfig : undefined,
        }));
        setPackageForm({
            name: '',
            description: '',
...
**Features**: ${Object.entries(appInfo.aiConfig.features).filter(([_, enabled]) => enabled).map(([feature]) => feature).join(', ')}
                        ...
                        {steps.map((step, index) => (
                            <div key={step.number} className="flex items-center flex-1">
                                <div className="flex flex-col items-center flex-1">
                                    <div
                                        className={`w-12 h-12 rounded-full flex items-center justify-center text-xl transition-all ${currentStep >= step.number
                                    ...
                                    <p className={`text-xs mt-2 text-center ${currentStep >= step.number
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
                                        Define your custom package details
                                    </p>
                                </div>
                                            ...
                                            <div className="pl-8 space-y-4 pt-4 border-t border-purple-200 dark:border-purple-700">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-3">
                                                        {[
                                                            ...
                                                            >
                                                                <input
                                                                    type="checkbox"
                                                                    checked={appInfo.aiConfig!.features[feature.key as keyof typeof appInfo.aiConfig.features]}
                                                                    onChange={(e) =>
                                                                        setAppInfo({
                                                                            ...appInfo,
                                                                            aiConfig: {
                                                ...
                                                <label className="flex items-center gap-2 cursor-pointer">
                                                    ...
                                                    <span className="text-sm">
                                                        Enable streaming responses (show AI responses in real-time)
                                                    </span>
                                                </label>
                                                ...
                                                <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
                                                    <div className="flex items-start gap-2">
                                                        ...
                                                        <div className="text-sm text-yellow-800 dark:text-yellow-200">
                                                            ...
                                                            <p className="text-xs">
                                                                You'll need to sign up with {appInfo.aiConfig.provider === 'anthropic' ? 'Anthropic' : appInfo.aiConfig.provider === 'openai' ? 'OpenAI' : 'Google'} and add your API key to the generated app. The generated code will include setup instructions. API costs are separate from AppForge credits.
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
                                ...
                                <div className="space-y-1 text-sm">
                                    <p><span className="font-medium">Name:</span> {appInfo.name}</p>
                                    <p><span className="font-medium">Category:</span> {appInfo.category}</p>
                                    <p><span className="font-medium">Target Audience:</span> {appInfo.targetAudience}</p>
                                    <p><span className="font-medium">Color Scheme:</span> {appInfo.colorScheme}</p>
                                </div>
                            </div>
                    ...
                    {currentStep === 2 && (
                        <div className="space-y-6">
                            <div>
                                ...
                                <p className="text-slate-600 dark:text-slate-400">
                                    Build a quick mood board so the rest of your design decisions feel cohesive.
                                    Drop in imagery, describe the tone, and capture keywords that we can reuse when
                                    generating your screens.
                                </p>
                            </div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                    {colorSchemes.map((scheme) => (
                                        <button
                                            ...
                                            className={`p-4 border-2 rounded-lg transition-all ${appInfo.colorScheme === scheme.value
                                                ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20'
                                                : 'border-slate-200 dark:border-slate-700 hover:border-slate-300'
                                                }`}
                                        >
                                ...
                                <div className="lg:col-span-2 space-y-3">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-sm text-slate-500">
                                                Pick a font personality that matches your product tone.
                                            </p>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        {TYPOGRAPHY_OPTIONS.map((option) => (
                                            <button
                                                ...
                                                className={`p-4 rounded-xl border-2 text-left transition-all ${styleBoard.typography === option.id
                                            ...
                                            >
                                                ...
                                                <p className="text-sm italic text-slate-600" style={{ fontFamily: option.id === 'serif-elegant' ? 'Georgia, serif' : option.id === 'playful-rounded' ? '"Nunito", "Comic Sans MS", sans-serif' : option.id === 'mono-tech' ? '"JetBrains Mono", "Courier New", monospace' : '"Inter", "Helvetica Neue", sans-serif' }}>
                                                    {option.sample}
                                                </p>
                                            </button>
                                        ))}
                                ...
                                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-inner">
                                    ...
                                    >
                                        ...
                                        <p
                                            ...
                                            style={{
                                                fontFamily: styleBoard.typography === 'serif-elegant'
                                                    ? 'Georgia, serif'
                                        ...
                                        >
                                            {styleBoard.uiStyle === 'playful' ? 'Playful & Friendly' : styleBoard.uiStyle === 'pro' ? 'Professional System' : styleBoard.uiStyle === 'edtech' ? 'Learning Hub' : 'Modern Dashboard'}
                                        </p>
                                        <p className="text-xs opacity-80">
                                            {styleBoard.personality.length > 0 ? styleBoard.personality.join(' · ') : 'Choose personality tags'}
                                        </p>
                                    </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            {UI_STYLE_OPTIONS.map((option) => (
                                                <button
                                                    ...
                                                    className={`p-3 rounded-lg border text-left text-sm ${styleBoard.uiStyle === option.id
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {PERSONALITY_OPTIONS.map((tag) => (
                                        <button
                                            ...
                                            className={`px-3 py-1 text-xs rounded-full border transition-all ${styleBoard.personality.includes(tag)
                                                ? 'border-blue-500 bg-blue-50 text-blue-700'
                                ...
                                <div className="flex flex-wrap gap-3">
                                    <div>
                                        ...
                                        <div className="flex gap-2">
                                            <Button
                                                type="button"
                                                variant={styleBoard.components.corners === 'rounded' ? 'default' : 'outline'}
                                            ...
                                            <Button
                                                type="button"
                                                variant={styleBoard.components.corners === 'sharp' ? 'default' : 'outline'}
                                    ...
                                    <div>
                                        ...
                                        <Button
                                            type="button"
                                            variant={styleBoard.components.gradient ? 'default' : 'outline'}
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex gap-2 flex-wrap">
                                        {styleBoard.keywords.map((keyword) => (
                                            ...
                                            >
                                                {keyword}
                                                <button onClick={() => removeKeyword(keyword)} className="text-blue-500 hover:text-blue-700">×</button>
                                            </span>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg p-4 space-y-2">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-500">Describe the story, emotions, or prompts that explain your desired look.</p>
                                    </div>
                                </div>
                                <textarea
                        ...
                        <div className="space-y-6">
                                ...
                                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-6 space-y-4">
                                    ...
                                    <div className="p-4 border border-blue-200 dark:border-blue-800 rounded-lg bg-blue-50 dark:bg-blue-900/20">
                                        ...
                                        {selectedPackageOption.description && (
                                            <p className="text-sm text-slate-600 dark:text-slate-400 mb-3">{selectedPackageOption.description}</p>
                                        )}
                                ...
                                <div className="bg-white dark:bg-slate-900 border border-purple-200 dark:border-purple-800 rounded-xl p-6 space-y-4">
                                    ...
                                    <div className="space-y-3 text-sm">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {Object.entries(appInfo.aiConfig.features)
                                                    ...
                                                    .map(([feature]) => (
                                                        <span key={feature} className="px-3 py-1 text-xs rounded-full bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className={`px-2 py-1 text-xs rounded ${appInfo.aiConfig.streaming ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-700'}`}>
                            ...
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-6 space-y-4">
                                ...
                                <div className="grid md:grid-cols-2 gap-4 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-medium">{styleBoard.components.corners === 'rounded' ? 'Rounded' : 'Sharp'} corners{styleBoard.components.gradient ? ', Gradient fills' : ', Solid fills'}</p>
                                    </div>
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    {savedLogo && (
                                        <div>
                                            ...
                                            <img src={savedLogo} alt="App Logo" className="w-32 h-32 object-contain border border-slate-200 dark:border-slate-700 rounded-lg" />
                                        </div>
                                    )}
                                    <div>
                                        <p className="text-slate-500 mb-2 text-sm">Screens</p>
                                        <p className="font-medium">{savedScreens.length} screen{savedScreens.length !== 1 ? 's' : ''} generated</p>
                                        <div className="flex flex-wrap gap-2 mt-2">
                                            {savedScreens.slice(0, 3).map((screen) => (
                                                <img key={screen.id} src={screen.url} alt={screen.name} className="w-16 h-28 object-cover border border-slate-200 dark:border-slate-700 rounded" />
                                            ))}
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                                <p className="text-sm text-blue-800 dark:text-blue-200">
                                    Ready to finalize? You can save your design package, download the PRD, or start building your app with AI.
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex justify-between mt-8 pt-6 border-t border-slate-200 dark:border-slate-700">
                        ...
                        {currentStep === 6 ? (
                                ...
                                <Button
                                    onClick={() => {
                                        ...
                                        if (!sessionId) {
                                            ...
                                            router.push(`/appbuild?sessionId=${sessionId}`);
                                        }
                                    }}
                            ...
                            <Button
                                ...
                                disabled={
                                    (currentStep === 1 &&
                                        (!appInfo.name || !appInfo.description || !appInfo.category || !appInfo.targetAudience || !appInfo.selectedPackage)) ||
