...
export function LogoGenerator({ appInfo, onSave, stylePreferences }: LogoGeneratorProps) {
    ...
    const generatePrompt = () => {
        ...
        if (stylePreferences?.uiStyle) {
            styleContextParts.push(UI_STYLE_DESCRIPTIONS[stylePreferences.uiStyle] || `${stylePreferences.uiStyle} interface style`);
        }
    ...
    const handleEdit = async () => {
        if (!editPrompt.trim() || !selectedLogo) return;
    ...
    const examplePrompts = [
        ...
        'minimalist productivity icon',
        'colorful social media app',
    ];
